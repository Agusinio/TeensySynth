...
void oscPlay(byte note) {
  ...
  waveform2.frequency(noteFreqs[note + octave2] * detuneFactor * bendFactor *
                      LFOpitch);
...
void oscStop() { envelope1.noteOff(); }

void oscSet() {
  waveform1.frequency(noteFreqs[globalNote + octave1] * bendFactor * LFOpitch);
  waveform2.frequency(noteFreqs[globalNote + octave2] * detuneFactor *
                      bendFactor * LFOpitch);
}
...
void myControlChange(byte channel, byte control, byte value) {
  ...
  case CCattack:
    envelope1.attack((3000 * (value * DIV127)) +
                     10.5); // TEST Attack min limit to 10.5ms
    break;
  ...
  if (currentMicros - LFOtime >= 1000) {
    ...
    if (muxRead > (muxValues[muxInput] + 7) ||
      ...
      case MUXosc1:
        muxRead = muxRead / 32;
        if (muxRead != osc1Mode)
          myControlChange(0, CCosc1, muxRead);
        break;
      case MUXosc2:
        muxRead = muxRead / 32;
        if (muxRead != osc2Mode)
          myControlChange(0, CCosc2, muxRead);
        break;
      case MUXdetune:
